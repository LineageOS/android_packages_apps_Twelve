...
class AmpacheDataSource(
    ...
    private class AmpacheInstance(val client: AmpacheClient, server: String) :
        ...
        fun org.lineageos.twelve.datasources.ampache.models.Album.toMediaItem() =
            Album.Builder(getAlbumUri(id))
                .setThumbnail(Thumbnail.Builder().setUri(art?.toUri()).build())
                .setTitle(name)
        ...
        fun org.lineageos.twelve.datasources.ampache.models.Genre.toMediaItem() =
            Genre.Builder(getGenreUri(id)).setName(name).build()

        fun org.lineageos.twelve.datasources.ampache.models.Lyrics.toModel() =
            Lyrics.Builder().apply { text.lines().forEach { line -> addLine(line) } }.build()
        ...
        fun getGenreUri(genreId: String): Uri = genresUri.buildUpon().appendPath(genreId).build()
    ...
    private val providersManager =
        ProvidersManager(coroutineScope, providersRepository, ProviderType.AMPACHE) {
            provider,
            arguments ->
                ...
                AmpacheClient(
                    ...
                    { token ->
                        database
                            ...
                            .updateToken(provider.typeId, token?.first, token?.second)
                    },
                    cache,
                )

            AmpacheInstance(ampacheClient, server)
        }
            ...
            client.ping().map { ping ->
                listOf(
                    DataSourceInformation(
                        key = "server",
                        keyLocalizedString =
                            LocalizedString.StringResIdLocalizedString(R.string.ampache_server),
                        value = LocalizedString.StringLocalizedString(ping.server),
                    ),
                    DataSourceInformation(
                        key = "version",
                        keyLocalizedString =
                            LocalizedString.StringResIdLocalizedString(R.string.ampache_version),
                        value = LocalizedString.StringLocalizedString(ping.version),
                    ),
                    DataSourceInformation(
                        key = "compatible",
                        keyLocalizedString =
                            LocalizedString.StringResIdLocalizedString(R.string.ampache_compatible),
    ...
    override fun activity(providerIdentifier: ProviderIdentifier) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            val mostPlayedAlbums =
                client.stats(type = "album", filter = "frequent", limit = 10).map { stats ->
                    ActivityTab(
            ...
            val randomAlbums =
                client.stats(type = "album", filter = "random", limit = 10).map { stats ->
                    ActivityTab(
                        id = "random_albums",
                        title =
                            LocalizedString.StringResIdLocalizedString(
            ...
            val randomSongs =
                client.stats(type = "song", filter = "random", limit = 10).map { stats ->
                    ActivityTab(
                        id = "random_songs",
                        title =
                            LocalizedString.StringResIdLocalizedString(
    ...
    override fun albums(providerIdentifier: ProviderIdentifier, sortingRule: SortingRule) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            client
                ...
                .map { albums -> albums.album.map { it.toMediaItem() } }
        }
    ...
    override fun artists(providerIdentifier: ProviderIdentifier, sortingRule: SortingRule) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            client
                ...
                .map { artists -> artists.artist.map { it.toMediaItem() } }
        }
    ...
    override fun genres(providerIdentifier: ProviderIdentifier, sortingRule: SortingRule) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            client
                ...
                .map { genres -> genres.genre.map { it.toMediaItem() } }
        }
                ...
                client
                    ...
                    .map { playlists ->
                        buildList {
                            ...
                            playlists.playlist.forEach { add(it.toMediaItem()) }
                        }
                    }
            }
        }
    ...
    override fun search(providerIdentifier: ProviderIdentifier, query: String) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            client
                .searchGroup("or", listOf(Triple("name", 0, "$query*")), type = "all", limit = 10)
                .map { search ->
                    buildList {
                        search.search.song?.forEach { add(it.toMediaItem()) }

                        search.search.album?.forEach { add(it.toMediaItem()) }

                        search.search.artist?.forEach { add(it.toMediaItem()) }

                        search.search.genre?.forEach { add(it.toMediaItem()) }

                        search.search.playlist?.forEach { add(it.toMediaItem()) }
                    }
                }
                .let { result ->
                    when (result) {
                        ...
                        is Result.Error ->
                            Result.Success(
                                buildList {
                                    client
                                        .songs(filter = query, exact = false, limit = 10)
                                        .getOrNull()
                                        ?.song
                                        ?.forEach { add(it.toMediaItem()) }

                                    client
                                        .albums(filter = query, exact = false, limit = 10)
                                        .getOrNull()
                                        ?.album
                                        ?.forEach { add(it.toMediaItem()) }

                                    client
                                        .artists(filter = query, exact = false, limit = 10)
                                        .getOrNull()
                                        ?.artist
                                        ?.forEach { add(it.toMediaItem()) }
                                }
    ...
    override fun audio(audioUri: Uri) =
        providersManager.flatMapWithInstanceOf(audioUri) {
            favoritesChanged.mapLatest {
                ...
                client.song(id).map { song -> song.toMediaItem() }
            }
        }
        ...
        providersManager.flatMapWithInstanceOf(genreUri) {
            favoritesChanged.mapLatest {
                ...
                client.genre(id).map { genre ->
                    genre.toMediaItem() to
                        GenreContent(
                            appearsInAlbums =
                                client.genreAlbums(id).getOrNull()?.album.orEmpty().map {
                                    it.toMediaItem()
                                },
                            appearsInPlaylists = listOf(),
                            audios =
                                client.genreSongs(id).getOrNull()?.song.orEmpty().map {
                                    it.toMediaItem()
                                },
                        )
                }
    ...
    override fun playlist(playlistUri: Uri) =
        providersManager.flatMapWithInstanceOf(playlistUri) {
            when (playlistUri) {
                favoritesUri ->
                    favoritesChanged.mapLatest {
                        client.stats(type = "song", filter = "flagged").map { stats ->
                    ...
                    playlistsChanged.mapLatest { _ ->
                        client.playlists().map { playlists ->
                            playlists.playlist.map { playlist ->
                                val inPlaylist =
                                    client.playlistSongs(playlist.id).map {
    ...
    override fun lyrics(audioUri: Uri) =
        providersManager.mapWithInstanceOf(audioUri) {
            client.getLyrics(audioUri.lastPathSegment!!, plugins = true).map { it.toModel() }
        }
        ...
        providersManager.doWithInstanceOf(playlistUri) {
            when (playlistUri) {
                favoritesUri -> Result.Error(Error.IO)
                    ...
                    client.playlistEdit(playlistUri.lastPathSegment!!, name = name).map {
                        onPlaylistsChanged()
                    }
            }
        }
        ...
        providersManager.doWithInstanceOf(playlistUri) {
            when (playlistUri) {
                favoritesUri -> Result.Error(Error.IO)
                    ...
                    client.playlistDelete(playlistUri.lastPathSegment!!).map {
                        onPlaylistsChanged()
                    }
            }
        }
    ...
    override suspend fun setFavorite(audioUri: Uri, isFavorite: Boolean) =
        providersManager.doWithInstanceOf(audioUri) {
            val id = audioUri.lastPathSegment!!
            client.flag(type = "song", filter = id, flag = isFavorite).map { onFavoritesChanged() }
        }

    /** Format a sort value for the Ampache API. */
    private fun formatSortValue(key: String?, reverse: Boolean) =
        key?.let { key ->
            val order =
                when (reverse) {
                    true -> "desc"
