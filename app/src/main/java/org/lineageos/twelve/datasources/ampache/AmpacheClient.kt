...
class AmpacheClient(
    ...
    private val interceptor =
        AmpacheInterceptor(username, password, applicationName, ::tokenProperty)

    private val okHttpClient =
        OkHttpClient.Builder().addInterceptor(interceptor).cache(cache).build()

    private val serverUri =
        server.toUri().buildUpon().appendPath("server").appendPath("json.server.php").build()
     ...
     * @param cond Apply additional filters to the browse using ; separated comma string pairs (e.g.
     *   'filter1,value1;filter2,value2')
     * @param sort Sort name or comma-separated key pair. (e.g. 'name,order') Default order 'ASC'
     *   (e.g. 'name,ASC' == 'name')
     */
    suspend fun albums(
        filter: String? = null,
     ...
     * @param includeSongs Whether to include songs in the response
     */
    suspend fun album(filter: String, includeSongs: Boolean = false) =
        action<Album>(
            "album",
            listOf(
                "filter" to filter,
     ...
     */
    suspend fun flag(type: String, filter: String, flag: Boolean) =
        action<Success>(
            "flag",
            listOf(
                "type" to type,
     ...
     */
    suspend fun genre(filter: String) = action<Genre>("genre", listOf("filter" to filter))
     ...
     */
    suspend fun getLyrics(filter: String, plugins: Boolean) =
        action<Lyrics>(
            "get_lyrics",
            listOf(
                "filter" to filter,
     ...
     */
    suspend fun playlist(filter: String) = action<Playlist>("playlist", listOf("filter" to filter))
     ...
     */
    suspend fun playlistAdd(filter: String, id: String, type: String, check: Boolean? = null) =
        action<Success>(
            "playlist_add",
            listOf(
                "filter" to filter,
     ...
     */
    suspend fun playlistCreate(name: String, type: Playlist.Type? = null) =
        action<Playlist>("playlist_create", listOf("name" to name, "type" to type?.value))
     ...
     * @param filter UID of Playlist
     */
    suspend fun playlistDelete(filter: String) =
        action<Success>("playlist_delete", listOf("filter" to filter))
     ...
     */
    suspend fun playlistRemoveSong(filter: String, song: String? = null, track: Int? = null) =
        action<Success>(
            "playlist_remove_song",
            listOf("filter" to filter, "song" to song, "track" to track),
        )
    ...
    suspend fun playlistSongs(
        ...
        action<Songs>(
            "playlist_songs",
            listOf("filter" to filter, "random" to random, "offset" to offset, "limit" to limit),
        )
     ...
     * @param type `all`, `music`, `song_artist`, `album_artist`, `podcast`, `video` (all by
     *   default)
    ...
    suspend fun searchGroup(
        ...
        action<Search>(
            ...
            listOf(
                ...
                rules
                    .mapIndexed { index, (rule, operator, input) ->
                        listOf(
     ...
     */
    suspend fun song(filter: String) = action<Song>("song", listOf("filter" to filter))
    ...
    private suspend inline fun <reified T> action(
        ...
        ApiRequest.get<T>(listOf(), listOf("action" to action) + parameters)
            .execute(api)
            .mapToError()
    ...
    companion object {
        ...
        fun calculatePassphrase(password: String, instant: Instant): String {
