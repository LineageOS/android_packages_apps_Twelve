...
class JellyfinClient(server: String, private val apiKey: String) {
    ...
    private suspend fun getItem(id: String) = method<BaseItemDto>(
        listOf("Items", id),
        null,
    )

    suspend fun getArtist(id: String) = getItem(id)
    suspend fun getAlbum(id: String) = getItem(id)

    suspend fun getAlbumTracks(id: String) = method<BaseItemDtoQueryResult>(
        listOf("Items"),
        null,
        listOf(
            "ParentId" to id,
