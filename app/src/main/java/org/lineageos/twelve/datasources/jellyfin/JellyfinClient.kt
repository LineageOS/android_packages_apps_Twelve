...
class JellyfinClient(
    ...
    suspend fun getAudios(sortingRule: SortingRule) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf("IncludeItemTypes" to "Audio", "Recursive" to true) +
    ...
    suspend fun getAlbums(sortingRule: SortingRule) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf("IncludeItemTypes" to "MusicAlbum", "Recursive" to true, "Limit" to 50) +
                        getSortParameter(sortingRule),
            )
            .execute(api)
            .mapToError()
    ...
    suspend fun getPlaylists(sortingRule: SortingRule) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf("IncludeItemTypes" to "Playlist", "Recursive" to true) +
                        getSortParameter(sortingRule),
            )
            .execute(api)
            .mapToError()

    suspend fun getItems(query: String) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf(
    ...
    suspend fun getAlbumTracks(id: UUID) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf("ParentId" to id, "IncludeItemTypes" to "Audio", "Recursive" to true),
            )
            .execute(api)
            .mapToError()

    suspend fun getArtistWorks(id: UUID) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf(
    ...
    suspend fun getPlaylistItemIds(id: UUID) =
        ApiRequest.get<PlaylistItems>(listOf("Playlists", id.toString())).execute(api).mapToError()

    suspend fun getPlaylistTracks(id: UUID) =
        ApiRequest.get<QueryResult>(listOf("Playlists", id.toString(), "Items"))
            .execute(api)
            .mapToError()

    suspend fun getGenreContent(id: UUID) =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf(
    ...
    fun getAudioPlaybackUrl(id: UUID) =
        api.buildUrl(
            listOf("Audio", id.toString(), "stream"),
            queryParameters = listOf("static" to true),
        )

    suspend fun getLyrics(id: UUID) =
        ApiRequest.get<Lyrics>(listOf("Audio", id.toString(), "lyrics")).execute(api).mapToError()

    suspend fun createPlaylist(name: String) =
        ApiRequest.post<CreatePlaylist, CreatePlaylistResult>(
                listOf("Playlists"),
                data =
                    CreatePlaylist(name = name, ids = listOf(), users = listOf(), isPublic = true),
            )
            .execute(api)
            .mapToError()

    suspend fun renamePlaylist(id: UUID, name: String) =
        ApiRequest.post<UpdatePlaylist, Unit>(
                listOf("Playlists", id.toString()),
                data = UpdatePlaylist(name = name),
            )
            .execute(api)
            .mapToError()

    suspend fun addItemToPlaylist(id: UUID, audioId: UUID) =
        ApiRequest.post<Unit, Unit>(
                listOf("Playlists", id.toString(), "Items"),
                queryParameters = listOf("Ids" to audioId),
            )
            .execute(api)
            .mapToError()

    suspend fun removeItemFromPlaylist(id: UUID, audioId: UUID) =
        ApiRequest.delete<Unit>(
                listOf("Playlists", id.toString(), "Items"),
                queryParameters = listOf("EntryIds" to audioId),
            )
            .execute(api)
            .mapToError()

    suspend fun getSystemInfo() =
        ApiRequest.get<SystemInfo>(listOf("System", "Info", "Public")).execute(api).mapToError()

    suspend fun addToFavorites(id: UUID) =
        ApiRequest.post<Unit, Unit>(listOf("UserFavoriteItems", id.toString()))
            .execute(api)
            .mapToError()

    suspend fun removeFromFavorites(id: UUID) =
        ApiRequest.delete<Unit>(listOf("UserFavoriteItems", id.toString()))
            .execute(api)
            .mapToError()

    suspend fun frequentlyPlayedAudio() =
        ApiRequest.get<QueryResult>(
                listOf("Items"),
                queryParameters =
                    listOf(
    ...
    suspend fun audioSuggestions() =
        ApiRequest.get<QueryResult>(
                listOf("Items", "Suggestions"),
                queryParameters = listOf("Type" to "Audio", "Limit" to 25),
            )
            .execute(api)
            .mapToError()

    suspend fun albumSuggestions() =
        ApiRequest.get<QueryResult>(
                listOf("Items", "Suggestions"),
                queryParameters = listOf("Type" to "MusicAlbum", "Limit" to 10),
            )
            .execute(api)
            .mapToError()
    ...
    suspend fun playlistSuggestions() =
        ApiRequest.get<QueryResult>(
                listOf("Items", "Suggestions"),
                queryParameters = listOf("Type" to "Playlist", "Limit" to 10),
            )
            .execute(api)
            .mapToError()

    suspend fun artistSuggestions() =
        ApiRequest.get<QueryResult>(
                listOf("Items", "Suggestions"),
                queryParameters = listOf("Type" to "MusicArtist", "Limit" to 10),
            )
            .execute(api)
            .mapToError()

    suspend fun broadcastPlaybackStart(itemId: UUID, positionTicks: Long = 0L) =
        ApiRequest.post<PlaybackStartInfo, Unit>(
                listOf("Sessions", "Playing"),
                data = PlaybackStartInfo(itemId.toString(), positionTicks),
            )
            .execute(api)
            .mapToError()

    private suspend fun getItem(id: UUID) =
        ApiRequest.get<Item>(listOf("Items", id.toString())).execute(api).mapToError()

    private fun getItemThumbnail(id: UUID) =
        api.buildUrl(
            listOf("Items", id.toString(), "Images", "Primary"),
            queryParameters = listOf("fillHeight" to "512", "fillWidth" to "512", "quality" to "96"),
        )
