...
class SoundCloudDataSource(
    ...
    override suspend fun setFavorite(
        ...
        when (val me = soundCloudClient.me()) {
            is Result.Success -> when (isFavorite) {
                true -> soundCloudClient.likeTrackForUser(audioId, me.data.id)
                else -> soundCloudClient.unlikeTrackForUser(audioId, me.data.id)
            }
            is Result.Error -> Result.Error(me.error, me.throwable)
        }
    }
