...
import org.lineageos.twelve.datasources.soundcloud.models.Playlist as SoundCloudSet
...
class SoundCloudClient(
    ...
    private val authInterceptor = SoundCloudAuthInterceptor(clientId, oAuthToken)
    ...
    suspend fun getLikedTracks(
        ...
        queryParameters = listOf(
            ...
            "linked_partitioning" to when (linkedPartitioning) {
                true -> "1"
                false -> "0"
            },
        )
    ...
    suspend fun getMixedSelections(
        ...
        offset: Int = 0,
        linkedPartitioning: Boolean = true,
    ) = ApiRequest.get<PaginatedCollection<Selection>>(
        listOf(
            "mixed-selections",
        ),
        queryParameters = listOf(
    ...
    suspend fun likeTrackForUser(
        trackId: Int,
        userId: Int,
    ) = ApiRequest.put<Any, String>(
        listOf(
            "users",
            "$userId",
            "track_likes",
    ...
    suspend fun unlikeTrackForUser(
        trackId: Int,
        userId: Int,
    ) = ApiRequest.delete<String>(
        listOf(
            "users",
            "$userId",
            "track_likes",
