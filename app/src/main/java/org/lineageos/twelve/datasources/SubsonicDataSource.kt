...
class SubsonicDataSource(arguments: Bundle) : MediaDataSource {
    ...
    override fun artists(sortingRules: List<SortingRule>) = suspend {
        subsonicClient.getArtists().toRequestStatus {
            index.flatMap { it.artist }.map { it.toMediaItem() }
        }
    }.asFlow()

    override fun genres(sortingRules: List<SortingRule>) = suspend {
        subsonicClient.getGenres().toRequestStatus {
            genre.map { it.toMediaItem() }
        }
    }.asFlow()

    override fun playlists(sortingRules: List<SortingRule>) = _playlistsChanged.mapLatest {
