...
class SubsonicDataSource(arguments: Bundle) : MediaDataSource {
    ...
    override fun albums(sortingRule: SortingRule) = suspend {
        val type = when (sortingRule.strategy) {
            SortingStrategy.CREATION_DATE -> "newest"
            SortingStrategy.NAME -> "alphabeticalByName"
            SortingStrategy.PLAY_COUNT -> "frequent"
            else -> null
        }
        ...
        subsonicClient.getAlbumList2(
            ...
            album.map { it.toMediaItem() }.asMaybeReversed(type != null && sortingRule.reverse)
        }
    }.asFlow()
