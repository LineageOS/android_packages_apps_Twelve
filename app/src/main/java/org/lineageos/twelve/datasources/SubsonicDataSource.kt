...
class SubsonicDataSource(
    ...
    private class SubsonicInstance(server: String, val subsonicClient: SubsonicClient) :
        ProvidersManager.Instance {
        private val dataSourceBaseUri = server.toUri()

        val albumsUri: Uri = dataSourceBaseUri.buildUpon().appendPath(ALBUMS_PATH).build()
        val artistsUri: Uri = dataSourceBaseUri.buildUpon().appendPath(ARTISTS_PATH).build()
        val audiosUri: Uri = dataSourceBaseUri.buildUpon().appendPath(AUDIOS_PATH).build()
        val genresUri: Uri = dataSourceBaseUri.buildUpon().appendPath(GENRES_PATH).build()
        val playlistsUri: Uri = dataSourceBaseUri.buildUpon().appendPath(PLAYLISTS_PATH).build()

        val favoritesUri: Uri = dataSourceBaseUri.buildUpon().appendPath(FAVORITES_PATH).build()
        ...
        fun org.lineageos.twelve.datasources.subsonic.models.Genre.toMediaItem() =
            Genre.Builder(getGenreUri(value)).setName(value).build()

        fun org.lineageos.twelve.datasources.subsonic.models.Playlist.toMediaItem() =
            Playlist.Builder(getPlaylistUri(id)).setName(name).build()

        fun org.lineageos.twelve.datasources.subsonic.models.MediaType?.toAudioType() =
            when (this) {
                org.lineageos.twelve.datasources.subsonic.models.MediaType.MUSIC -> Audio.Type.MUSIC
                org.lineageos.twelve.datasources.subsonic.models.MediaType.PODCAST ->
                    Audio.Type.PODCAST
                org.lineageos.twelve.datasources.subsonic.models.MediaType.AUDIOBOOK ->
                    Audio.Type.AUDIOBOOK
                org.lineageos.twelve.datasources.subsonic.models.MediaType.VIDEO ->
                    throw Exception("Invalid media type, got VIDEO")
                ...
                Lyrics.Builder()
                    .apply {
                        structuredLyrics.line.forEach { line ->
                            ...
                            addLine(text = line.value, startMs = startMs)
                        }
                    }
                    .build()
            }

        fun getAlbumUri(albumId: String): Uri = albumsUri.buildUpon().appendPath(albumId).build()

        fun getArtistUri(artistId: String): Uri =
            artistsUri.buildUpon().appendPath(artistId).build()

        fun getAudioUri(audioId: String): Uri = audiosUri.buildUpon().appendPath(audioId).build()

        fun getGenreUri(genre: String): Uri = genresUri.buildUpon().appendPath(genre).build()
    ...
    private val providersManager =
        ProvidersManager(coroutineScope, providersRepository, ProviderType.SUBSONIC) { _, arguments
            ->
            ...
            val subsonicClient =
                SubsonicClient(server, username, password, "Twelve", useLegacyAuthentication, cache)

            SubsonicInstance(server, subsonicClient)
        }
            ...
            ping.map {
                listOfNotNull(
                    DataSourceInformation(
                        "version",
                        LocalizedString.StringResIdLocalizedString(R.string.subsonic_version),
        ...
        providersManager
            .doWithInstanceOf(mediaItemUri) {
                with(mediaItemUri.toString()) {
                    ...
                    }?.let { Result.Success(it) } ?: Result.Error(Error.NOT_FOUND)
                }
            }
            .getOrNull()
    ...
    override fun activity(providerIdentifier: ProviderIdentifier) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            val mostPlayedAlbums =
                subsonicClient.getAlbumList2("frequent", 10).map { albumList2 ->
                    ActivityTab(
                        "most_played_albums",
                        LocalizedString.StringResIdLocalizedString(
            ...
            val randomAlbums =
                subsonicClient.getAlbumList2("random", 10).map { albumList2 ->
                    ActivityTab(
                        "random_albums",
                        LocalizedString.StringResIdLocalizedString(R.string.activity_random_albums),
                        albumList2.album.map { it.toMediaItem() },
                    )
                }

            val randomSongs =
                subsonicClient.getRandomSongs(20).map { songs ->
                    ActivityTab(
                        "random_songs",
                        LocalizedString.StringResIdLocalizedString(R.string.activity_random_songs),
                        songs.song.map { it.toMediaItem() },
                    )
                }

            Result.Success(
                listOf(mostPlayedAlbums, randomAlbums, randomSongs).mapNotNull {
    ...
    override fun albums(providerIdentifier: ProviderIdentifier, sortingRule: SortingRule) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            subsonicClient.getAlbumList2("alphabeticalByName", 500).map { albumList2 ->
                albumList2.album
                    .maybeSortedBy(
                        sortingRule.reverse,
                        ...
                        playlists.playlist
                            .maybeSortedBy(
                                ...
                                when (sortingRule.strategy) {
                                    ...
                                    SortingStrategy.NAME -> { playlist -> playlist.name }

                                    else -> null
                                },
                            )
                            .forEach { add(it.toMediaItem()) }
                    }
                }
            }
        }
    ...
    override fun album(albumUri: Uri) =
        providersManager.mapWithInstanceOf(albumUri) {
            subsonicClient.getAlbum(albumUri.lastPathSegment!!).map { albumWithSongsID3 ->
                albumWithSongsID3.toAlbumID3().toMediaItem() to
                    albumWithSongsID3.song.map { it.toMediaItem() }
            }
        }
    ...
    override fun artist(artistUri: Uri) =
        providersManager.mapWithInstanceOf(artistUri) {
            subsonicClient.getArtist(artistUri.lastPathSegment!!).map { artistWithAlbumsID3 ->
                artistWithAlbumsID3.toArtistID3().toMediaItem() to
                    ArtistWorks(
    ...
    override fun genre(genreUri: Uri) =
        providersManager.mapWithInstanceOf(genreUri) {
            val genreName = genreUri.lastPathSegment!!

            val appearsInAlbums =
                subsonicClient
                    .getAlbumList2("byGenre", size = 500, genre = genreName)
                    .map { albumList2 -> albumList2.album.map { it.toMediaItem() } }
                    .let {
                        when (it) {
            ...
            val audios =
                subsonicClient
                    ...
                    .map { songs -> songs.song.map { it.toMediaItem() } }
                    .let {
                        when (it) {
                            is Result.Success -> it.data
                            else -> null
            ...
            val exists = listOf(appearsInAlbums, audios).any { it != null }

            if (exists) {
                Result.Success(
                    Genre.Builder(genreUri).setName(genreName).build() to
                        GenreContent(appearsInAlbums.orEmpty(), listOf(), audios.orEmpty())
                )
            } else {
                ...
                playlistUri == favoritesUri ->
                    favoritesChanged.mapLatest {
                        subsonicClient.getStarred2().map {
                            favoritesPlaylist to
                                it.song.orEmpty().map { child -> child.toMediaItem() }
                        }
                    }
                    ...
                    playlistsChanged.mapLatest {
                        subsonicClient.getPlaylist(playlistUri.lastPathSegment!!).map {
                            it.toPlaylist().toMediaItem() to
                                it.entry.orEmpty().map { child -> child.toMediaItem() }
                        }
                    }
            }
        }
    ...
    override fun audioPlaylistsStatus(audioUri: Uri) =
        audioUri.lastPathSegment!!.let { audioId ->
            providersManager.flatMapWithInstanceOf(audioUri) {
                combine(
                    ...
                    playlistsChanged.mapLatest { _ ->
                        subsonicClient.getPlaylists().map { playlists ->
                            playlists.playlist.map { playlist ->
                                val inPlaylist =
                                    subsonicClient.getPlaylist(playlist.id).map {
    ...
    override suspend fun createPlaylist(providerIdentifier: ProviderIdentifier, name: String) =
        providersManager.doWithInstanceOf(providerIdentifier) {
            subsonicClient.createPlaylist(null, name, listOf()).map { playlistWithSongs ->
                onPlaylistsChanged()
        ...
        providersManager.doWithInstanceOf(playlistUri) {
            when {
                playlistUri == favoritesUri -> Result.Error(Error.IO)
                    ...
                    subsonicClient.deletePlaylist(playlistUri.lastPathSegment!!).map {
                        onPlaylistsChanged()
                    }
            }
        }
    ...
    override suspend fun setFavorite(audioUri: Uri, isFavorite: Boolean) =
        providersManager.doWithInstanceOf(audioUri) {
            ...
            }.map { onFavoritesChanged() }
        }

    /**
     * Apply [List.asReversed] if [condition] is true. Reminder that [List.asReversed] returns a new
     * list view, thus being O(1).
     */
    private fun <T> List<T>.asMaybeReversed(condition: Boolean) =
        when (condition) {
            true -> asReversed()
            else -> this
        }
    ...
    /**
     * Sort this list by the [selector] and apply [List.asReversed] if [reverse] is true. If
     * [selector] is null, return the original list.
     */
    private fun <T> List<T>.maybeSortedBy(reverse: Boolean, selector: ((T) -> Comparable<*>?)?) =
        selector?.let {
    ...
    companion object {
            ...
            ProviderArgument(
                ...
                validate = {
                    when (it.toHttpUrlOrNull()) {
                        null ->
                            ProviderArgument.ValidationError(
                                "Invalid URL",
        ...
        val ARG_USE_LEGACY_AUTHENTICATION =
            ProviderArgument(
