...
class JellyfinDataSource(
    ...
    private class JellyfinInstance(val client: JellyfinClient, val server: String) :
        ...
        fun Item.toMediaItemAlbum() =
            Album.Builder(getAlbumUri(id.toString()))
                .setThumbnail(
                    Thumbnail.Builder().setUri(client.getAlbumThumbnail(id).toUri()).build()
                )
                .setTitle(name)
        ...
        fun Item.toMediaItemArtist() =
            Artist.Builder(getArtistUri(id.toString()))
                .setThumbnail(
                    Thumbnail.Builder().setUri(client.getArtistThumbnail(id).toUri()).build()
                )
                .setName(name)
                .build()

        fun Item.toMediaItemAudio() =
            Audio.Builder(getAudioUri(id.toString()))
        ...
        fun Item.toMediaItemGenre() =
            Genre.Builder(getGenreUri(id.toString()))
                .setThumbnail(
                    Thumbnail.Builder().setUri(client.getGenreThumbnail(id).toUri()).build()
                )
                .setName(name)
                .build()

        fun Item.toMediaItemPlaylist() =
            Playlist.Builder(getPlaylistUri(id.toString()))
                .setThumbnail(
                    Thumbnail.Builder().setUri(client.getPlaylistThumbnail(id).toUri()).build()
                )
                .setName(name)
                .build()

        fun org.lineageos.twelve.datasources.jellyfin.models.Lyrics.toModel() =
            lyrics?.let {
                Lyrics.Builder()
                    .apply {
                        it.forEach { lyrics ->
                            addLine(text = lyrics.text, startMs = lyrics.start / 10000)
                        }
                    }
                    .build()
            }
    ...
    private val providersManager =
        ProvidersManager(coroutineScope, providersRepository, ProviderType.JELLYFIN) {
            provider,
            arguments ->
    ...
    override fun activity(providerIdentifier: ProviderIdentifier) =
        providersManager.mapWithInstanceOf(providerIdentifier) {
            val frequentlyPlayedSongs =
                client.frequentlyPlayedAudio().map { queryResult ->
                    ActivityTab(
            ...
            val randomSongs =
                client.audioSuggestions().map { queryResult ->
                    ActivityTab(
                        "random_songs",
            ...
            val randomAlbums =
                client.albumSuggestions().map { queryResult ->
                    ActivityTab(
                        "random_albums",
            ...
            val randomArtists =
                client.artistSuggestions().map { queryResult ->
                    ActivityTab(
                        "random_artists",
                        LocalizedString.StringResIdLocalizedString(
            ...
            val randomPlaylists =
                client.playlistSuggestions().map { queryResult ->
                    ActivityTab(
                        "random_playlists",
                        LocalizedString.StringResIdLocalizedString(
    ...
    override fun audio(audioUri: Uri) =
        providersManager.flatMapWithInstanceOf(audioUri) {
            favoritesChanged.mapLatest {
                ...
                client.getAudio(id).map { it.toMediaItemAudio() }
            }
        }
        ...
        providersManager.mapWithInstanceOf(albumUri) {
            ...
            client.getAlbum(id).map { item ->
                val tracks =
                    client
                        .getAlbumTracks(id)
                        .map { queryResult -> queryResult.items.map { it.toMediaItemAudio() } }
                        .getOrNull()
                        .orEmpty()
            ...
            client.getGenre(id).map { item ->
                ...
                item.toMediaItemGenre() to
                    GenreContent(
                        ...
                        audios =
                            items.filter { it.type == ItemType.AUDIO }.map { it.toMediaItemAudio() },
                    )
            }
        }
                    ...
                    playlistsChanged.mapLatest {
                        ...
                        client.getPlaylists(sortingRule).map { queryResult ->
                            queryResult.items.map { playlist ->
                                val playlistItems =
                                    client
                                        .getPlaylistItemIds(playlist.id)
                                        .map { it.itemIds }
                                        .getOrNull()
                                        .orEmpty()
        ...
        providersManager.doWithInstanceOf(playlistUri) {
            when {
                playlistUri == favoritesUri -> Result.Error(Error.IO)
                    ...
                    client
                        .renamePlaylist(UUID.fromString(playlistUri.lastPathSegment!!), name)
        ...
        providersManager.doWithInstanceOf(playlistUri, audioUri) {
            when {
                playlistUri == favoritesUri -> setFavorite(audioUri, true)
                    ...
                    client.addItemToPlaylist(playlistId, audioId).map { onPlaylistsChanged() }
                }
            }
        }
        ...
        providersManager.doWithInstanceOf(playlistUri, audioUri) {
            when {
                playlistUri == favoritesUri -> setFavorite(audioUri, false)
                    ...
                    val playlistId = UUID.fromString(playlistUri.lastPathSegment!!)
                    val audioId = UUID.fromString(audioUri.lastPathSegment!!)
                    client.removeItemFromPlaylist(playlistId, audioId).map { onPlaylistsChanged() }
                }
            }
        }

    override suspend fun onAudioPlayed(audioUri: Uri, positionMs: Long) =
        providersManager.doWithInstanceOf(audioUri) {
            val itemId = UUID.fromString(audioUri.lastPathSegment!!)
            client.broadcastPlaybackStart(itemId = itemId, positionTicks = positionMs)
        }
