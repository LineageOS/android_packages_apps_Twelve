...
class JellyfinDataSource(arguments: Bundle) : MediaDataSource {
    ...
    override fun playlists(sortingRule: SortingRule) =
        emptyFlow<MediaRequestStatus<List<Playlist>>>()

    override fun search(query: String) = emptyFlow<MediaRequestStatus<List<MediaItem<*>>>>()
    ...
    override fun artist(artistUri: Uri) = emptyFlow<MediaRequestStatus<Pair<Artist, ArtistWorks>>>()
