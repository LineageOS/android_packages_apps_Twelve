...
class JellyfinDataSource(context: Context, arguments: Bundle, cache: Cache) : MediaDataSource {
    ...
    private val client = JellyfinClient(
        server, username, password, deviceIdentifier, packageName, cache
    )
    ...
    override fun audioPlaylistsStatus(audioUri: Uri) = _playlistsChanged.mapLatest {
        ...
        // There's probably a smarter way to query this, but this is the simplest way
        client.getPlaylists(sortingRule).toRequestStatus {
            items.map { playlist ->
                playlist.toMediaItemPlaylist() to (client.getPlaylistTracks(playlist.id).toResult {
                    items.any { it.id == audioId }
                } ?: false)
            }
        }
    }
