...
class SubsonicAuthInterceptor(
    ...
    override fun intercept(chain: Interceptor.Chain) = chain.proceed(
        chain.request().newBuilder()
            .url(
                chain.request().url.addQueryParameters(authParameters).build()
            )
            .build()
    )

    private fun HttpUrl.addQueryParameters(queryParameters: List<Pair<String, Any>>): HttpUrl.Builder {
        val url = newBuilder()
    ...
    private val authParameters = buildList {
        add("u" to username)
