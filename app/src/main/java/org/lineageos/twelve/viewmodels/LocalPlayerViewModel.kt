...
class LocalPlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val durationCurrentPositionMs = exoPlayer.playbackProgressFlow(eventsFlow)
        .flatMapLatest { playbackProgress ->
            flow {
                while (true) {
                    val duration = playbackProgress.durationMs.takeIf { it != C.TIME_UNSET }
                    val currentPosition =
                        exoPlayer.currentPosition.takeIf { duration != C.TIME_UNSET }
                    emit(duration to duration?.let { currentPosition?.coerceAtMost(duration) })
                    delay(200)
                }
            }
        }
