...
interface AmpacheProviderDao {
    ...
    suspend fun create(name: String, url: String, username: String, password: String): Long
    ...
    @Query("SELECT * FROM AmpacheProvider") fun getAll(): Flow<List<AmpacheProvider>>
    ...
    @Query(
        "SELECT token_expiration FROM AmpacheProvider WHERE ampache_provider_id = :ampacheProviderId"
    )
