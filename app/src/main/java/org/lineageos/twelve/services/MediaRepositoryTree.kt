...
class MediaRepositoryTree(
    ...
    private val noPermissionsMediaItem =
        buildMediaItem(
    ...
    private val noPermissionsDescriptionMediaItem =
        buildMediaItem(
    ...
    private val changeProviderMediaItem =
        buildMediaItem(
    ...
    private val providerChangedMediaItem =
        buildMediaItem(
    ...
    suspend fun getItem(mediaId: String) =
        when (mediaId) {
    ...
    suspend fun getChildren(mediaId: String) =
        when (mediaId) {
            ROOT_MEDIA_ITEM_ID ->
                when (context.permissionsGranted(PermissionsUtils.mainPermissions)) {
                    true ->
                        listOf(
                            albumsMediaItem,
                            artistsMediaItem,
            ...
            ALBUMS_MEDIA_ITEM_ID ->
                repository.albums().toOneShotResult().map {
                    it.toMedia3MediaItem(context.resources)
                }

            ARTISTS_MEDIA_ITEM_ID ->
                repository.artists().toOneShotResult().map {
                    it.toMedia3MediaItem(context.resources)
                }

            GENRES_MEDIA_ITEM_ID ->
                repository.genres().toOneShotResult().map {
                    it.toMedia3MediaItem(context.resources)
                }

            PLAYLISTS_MEDIA_ITEM_ID ->
                repository.playlists().toOneShotResult().map {
                    it.toMedia3MediaItem(context.resources)
                }

            CHANGE_PROVIDER_MEDIA_ITEM_ID ->
                providersRepository.allProviders.first().map { it.toMedia3MediaItem() }
                ...
                when {
                    mediaId.startsWith(PROVIDER_MEDIA_ITEM_ID_PREFIX) -> {
                        ...
                        mediaIdToMediaItemUri(mediaId)?.let { mediaItemUri ->
                            when (mediaItemUriToMediaType(mediaItemUri)) {
                                MediaType.ALBUM ->
                                    repository.album(mediaItemUri).toOneShotResult().second.map {
                                        albumAudios ->
                                        albumAudios.toMedia3MediaItem(context.resources)
                                    }

                                MediaType.ARTIST ->
                                    repository.artist(mediaItemUri).toOneShotResult().second.let {
                                        artistWorks ->
                                        listOf(artistWorks.albums, artistWorks.appearsInAlbum)
                                            .flatten()
                                            .map { allRelatedAlbums ->
                                ...
                                MediaType.GENRE ->
                                    repository.genre(mediaItemUri).toOneShotResult().second.let {
                                        genreContent ->
                                        listOf(
    ...
    suspend fun resolveMediaItems(mediaItems: List<MediaItem>) = buildList {
        mediaItems.forEach { item ->
            ...
            when {
                ...
                else -> getItem(item.mediaId)?.let { mediaItem -> add(mediaItem) }
            }
        }
    }
    ...
    suspend fun search(query: String) =
        repository.search("%${query}%").toOneShotResult().map {
            it.toMedia3MediaItem(context.resources)
        }
    ...
    private fun mediaIdToMediaItemUri(mediaId: String) = runCatching { mediaId.toUri() }.getOrNull()

    private suspend fun mediaItemUriToMediaType(mediaItemUri: Uri) =
        repository.mediaTypeOf(mediaItemUri)

    /** Given a media ID, get the item from the repository. */
    private suspend fun mediaIdToMediaItem(mediaId: String) =
    ...
    private fun Provider.toMedia3MediaItem() =
        buildMediaItem(
            title = name,
    ...
    private suspend fun mediaIdToProvider(mediaId: String): Provider? {
        ...
        return providersRepository
            .provider(ProviderIdentifier(providerType, providerTypeId))
            .first()
    }
    ...
    companion object {
        ...
        /**
         * Converts a flow of [Result] to a one-shot result of [T]. On success, returns the data. On
         * error, logs the exception and returns null.
         */
