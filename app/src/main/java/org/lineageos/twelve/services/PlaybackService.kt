...
class PlaybackService : MediaLibraryService(), LifecycleOwner {
    ...
    private fun getCustomLayout() = listOf(
        ButtonCommand.getShuffleModeButton(player.shuffleModeEnabled).commandButton(resources),
        ButtonCommand.getRepeatModeButton(player.repeatMode).commandButton(resources),
    )
}
