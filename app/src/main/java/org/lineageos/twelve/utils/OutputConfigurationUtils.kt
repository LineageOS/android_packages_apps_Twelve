...
object OutputConfigurationUtils {
    ...
    fun Format.toModel(): OutputConfiguration.Source {
        ...
        return OutputConfiguration.Source(
            ...
            peakBitrate = peakBitrate.takeIf { it != Format.NO_VALUE },
            averageBitrate = averageBitrate.takeIf { it != Format.NO_VALUE },
        )
    }
    ...
    fun buildOutputTranscoding(
        ...
        return OutputConfiguration.Transcoding(
            outputMode = outputModeFromMedia3OutputMode(
                transcodingData?.outputMode ?: return null
            ),
            encoding = transcodingData.encoding?.let { encodingFromMedia3Encoding(it) },
            sampleRate = audioFormat?.sampleRate?.takeIf {
                it != AudioFormat.SAMPLE_RATE_UNSPECIFIED
            },
            channelCount = audioFormat?.channelCount?.takeIf { it != 0 },
            bitrate = transcodingData.bitrate.takeIf { it != Format.NO_VALUE },
        )
    }
