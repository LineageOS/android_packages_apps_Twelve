...
object OutputConfigurationUtils {
    ...
    private val mimeTypeToCompression =
        mapOf(
    ...
    private val pcmEncodingBitDepths =
        mapOf(
    ...
    fun AudioDeviceInfo.toModel() =
        OutputConfiguration.Device(
            ...
            encodings =
                encodings.toList().mapNotNull { encodingFromAudioFormatEncoding(it) }.toSet(),
        )
    ...
    private fun classify(
        ...
        val verdict =
            OutputConfiguration.Verdict(hiRes = isHiRes, potentialIssues = potentialIssues)
    ...
    private fun Format.guessEncoding(): OutputConfiguration.Encoding? {
        ...
        listOfNotNull(MimeTypes.getAudioMediaMimeType(codecs), sampleMimeType, containerMimeType)
            .forEach { mimeType ->
    ...
    private fun Format.getStreamMimeType() =
        MimeTypes.getAudioMediaMimeType(codecs) ?: sampleMimeType ?: containerMimeType

    private fun encodingFromMedia3Encoding(media3Encoding: @C.Encoding Int) =
        when (media3Encoding) {
            Format.NO_VALUE,
    ...
    private fun encodingFromAudioFormatEncoding(audioFormatEncoding: Int) =
    ...
    private fun outputModeFromAudioTrackConfig(audioTrackConfig: AudioSink.AudioTrackConfig) =
        when {
            audioTrackConfig.offload -> OutputMode.OFFLOAD

            else ->
                when (audioTrackConfig.encoding) {
                    C.ENCODING_PCM_8BIT,
                    C.ENCODING_PCM_16BIT,
                    C.ENCODING_PCM_24BIT,
                    C.ENCODING_PCM_32BIT,
