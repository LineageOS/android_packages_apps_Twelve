...
class PlaybackControlBottomSheetDialogFragment :
    TwelveBottomSheetDialogFragment(R.layout.fragment_playback_control_bottom_sheet_dialog) {
    // View models
    ...
    private val playbackSpeedMinusMaterialButton by
        getViewProperty<MaterialButton>(R.id.playbackSpeedMinusMaterialButton)
    private val playbackSpeedPlusMaterialButton by
        getViewProperty<MaterialButton>(R.id.playbackSpeedPlusMaterialButton)
    private val playbackPitchSlider by getViewProperty<Slider>(R.id.playbackPitchSlider)
    private val playbackPitchUnlockMaterialSwitch by
        getViewProperty<MaterialSwitch>(R.id.playbackPitchUnlockMaterialSwitch)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        playbackSpeedMinusMaterialButton.setOnClickListener { viewModel.decreasePlaybackSpeed() }

        playbackSpeedMaterialButton.setOnClickListener { viewModel.resetPlaybackSpeed() }

        playbackSpeedPlusMaterialButton.setOnClickListener { viewModel.increasePlaybackSpeed() }
        ...
        require((sliderTo - sliderFrom).toInt() % 2 == 0) { "Slider range must have an odd length" }

        playbackPitchSlider.addOnChangeListener { _, value, _ ->
            viewModel.setPlaybackPitch(
                PlaybackControlViewModel.sliderToPitch(value, sliderFrom, sliderTo)
            )
        }

        playbackPitchSlider.setLabelFormatter {
            playbackPitchFormatter.format(
                PlaybackControlViewModel.sliderToPitch(it, sliderFrom, sliderTo)
            )
        }
