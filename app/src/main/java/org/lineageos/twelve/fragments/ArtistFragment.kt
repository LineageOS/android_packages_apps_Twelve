...
class ArtistFragment : CollapsingToolbarLayoutFragment(R.layout.fragment_artist) {
    ...
    private val appearsInAlbumLinearLayout by
        getViewProperty<LinearLayout>(R.id.appearsInAlbumLinearLayout)
    private val appearsInAlbumRecyclerView by
        getViewProperty<RecyclerView>(R.id.appearsInAlbumRecyclerView)
    private val appearsInPlaylistLinearLayout by
        getViewProperty<LinearLayout>(R.id.appearsInPlaylistLinearLayout)
    private val appearsInPlaylistRecyclerView by
        getViewProperty<RecyclerView>(R.id.appearsInPlaylistRecyclerView)
        ...
        viewModel.artist.collectLatest {
            ...
            when (it) {
                ...
                is FlowResult.Success -> {
                    ...
                    val isEmpty =
                        listOf(isAlbumsEmpty, isAppearsInAlbumEmpty, isAppearsInPlaylistEmpty)
                            .all { isEmpty -> isEmpty }
    ...
    companion object {
         ...
         */
        fun createBundle(artistUri: Uri) = Bundle { putParcelable(ARG_ARTIST_URI, artistUri) }
    }
}
