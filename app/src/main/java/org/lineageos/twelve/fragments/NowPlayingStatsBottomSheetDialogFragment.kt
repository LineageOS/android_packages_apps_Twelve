            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                launch {
                    viewModel.outputConfiguration.collectLatest { outputConfiguration ->
                        // Verdict
                        outputConfiguration?.verdict?.potentialIssues.orEmpty().let {
                        ...
                        outputConfiguration?.source?.sampleRateHz?.also { sampleRateHz ->
                            sourceSampleRateListItem.setSupportingText(
                                R.string.audio_sample_rate_format,
                                decimalFormatter.format(sampleRateHz / 1000f)
                            )
                        } ?: sourceSampleRateListItem.setSupportingText(
                        ...
                        outputConfiguration?.source?.peakBitrateBps?.also { peakBitrateBps ->
                        ...
                        outputConfiguration?.source?.averageBitrateBps?.also { averageBitrateBps ->
                        ...
                        outputConfiguration?.transcoding?.sampleRateHz?.also { sampleRateHz ->
                        ...
                        outputConfiguration?.transcoding?.channelCount?.also { channelCount ->
                            transcodingChannelCountListItem.supportingText = channelCount.toString()
                            transcodingChannelCountListItem.isVisible = true
                        } ?: run {
                            transcodingChannelCountListItem.isVisible = false
                        }

                        outputConfiguration?.transcoding?.bitrateBps?.also { bitrateBps ->
                            transcodingBitrateListItem.setSupportingText(
                                R.string.audio_bitrate_format,
                                decimalFormatter.format(bitrateBps / 1000f)
                            )
                        ...
                        outputConfiguration?.device?.sampleRatesHz?.maxOrNull()
                            ?.also { sampleRate ->
                                deviceMaxSampleRateListItem.setSupportingText(
                                    R.string.audio_sample_rate_format,
                                    decimalFormatter.format(sampleRate / 1000f)
                                )
