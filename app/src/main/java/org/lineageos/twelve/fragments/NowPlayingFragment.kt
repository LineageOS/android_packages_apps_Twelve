    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                ...
                launch {
                    viewModel.currentLyricsLines.collectLatest {
                        it?.let { (lyrics, currentIndex) ->
                            val previousLyrics = lyrics.getOrNull(currentIndex - 1)
                            val currentLyrics = lyrics.getOrNull(currentIndex)
                            val nextLyrics = lyrics.getOrNull(currentIndex + 1)

                            previousLyricsTextView.text = previousLyrics?.line
                            currentLyricsTextView.text = currentLyrics?.line
                            nextLyricsTextView.text = nextLyrics?.line
                        }
                    }
                }
            }
