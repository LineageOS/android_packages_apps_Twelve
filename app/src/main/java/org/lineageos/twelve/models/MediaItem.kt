...
sealed interface MediaItem<T : MediaItem<T>> : UniqueItem<T> {
     ...
     */
    abstract class Builder<B : Builder<B, T>, T : MediaItem<T>>(protected val uri: Uri) {
        protected var thumbnail: Thumbnail? = null

        /** @see MediaItem.thumbnail */
        fun setThumbnail(thumbnail: Thumbnail?) = self.also { this.thumbnail = thumbnail }

        /** Build the [MediaItem]. */
        abstract fun build(): T

        private val self: B
            @Suppress("UNCHECKED_CAST") get() = this as B
    }
}
