    ...
    class Builder {
        ...
        fun build() = Lyrics(
            lines = lines.sortedBy { it.startMs }.let { sortedLines ->
                val startDurations = sortedLines.mapNotNull { it.startMs }.distinct().asReversed()

                sortedLines.map { line ->
                    val endMs = when {
                        line.endMs != null -> line.endMs
                        line.startMs != null -> startDurations.lastOrNull { startDuration ->
                            startDuration > line.startMs
                        }?.minus(1L)
                        else -> null
                    }
